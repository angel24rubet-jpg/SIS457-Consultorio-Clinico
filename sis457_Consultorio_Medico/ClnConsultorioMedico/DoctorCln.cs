    ...
    public class DoctorCln
    {
        ...
        public static List<paDoctorListar_Result> listarPa(string parametro)
        {
            using (var context = new LabConsultorioMedicoEntities())
            {
                ...
                // Filtrado en memoria: buscar por nombre, cédula o especialidad (campo `especialidad` viene del SP)
                return lista
                    .Where(x =>
                        (!string.IsNullOrEmpty(x.nombreCompletoDoctor) &&
                         x.nombreCompletoDoctor.IndexOf(parametro, StringComparison.CurrentCultureIgnoreCase) >= 0)
                        || (!string.IsNullOrEmpty(x.cedulaIdentidad) &&
                         x.cedulaIdentidad.IndexOf(parametro, StringComparison.CurrentCultureIgnoreCase) >= 0)
                        || (!string.IsNullOrEmpty(x.especialidad) &&
                         x.especialidad.IndexOf(parametro, StringComparison.CurrentCultureIgnoreCase) >= 0)
                    )
                    .ToList();
            }
        }
